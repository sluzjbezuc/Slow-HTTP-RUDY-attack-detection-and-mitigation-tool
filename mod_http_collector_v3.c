#include "mod_http_collector.h"  /* Include the header file first */

#include "util_filter.h"
#include "util_time.h"

/* Global variables for shared memory and concurrency tracking */
int test_mode = 0;
...
static int http_collector_log_phase(request_rec *r)
{
    ...
    rm->connection_duration =
        (apr_time_as_msec(rm->request_completion_time)
         - apr_time_as_msec(rm->request_initiation_time));
...
/* Implementation of get_concurrency declared in the header */
