...
apr_shm_t        *g_shm         = NULL;
concurrency_shm_t *g_concurrency = NULL;
...
AP_DECLARE(int) get_concurrency(const char *ip) {
    if (!g_concurrency) {
        return -1;
    }

    apr_thread_mutex_lock(g_concurrency->table_mutex);
    concurrency_entry_t *entry = find_or_create_concurrency_entry(ip);
    int concurrency = (entry) ? entry->active_connections : -1;
    apr_thread_mutex_unlock(g_concurrency->table_mutex);

    return concurrency;
}

/* Function to retrieve request_metrics_t for a given request */
AP_DECLARE(request_metrics_t*) get_request_metrics(request_rec *r)
{
    if (!r) {
        return NULL;
    }
    return (request_metrics_t *)ap_get_module_config(r->request_config, &http_collector_module);
}
