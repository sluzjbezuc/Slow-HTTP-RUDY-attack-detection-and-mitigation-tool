...
typedef struct {
    ...
    int active_connections; // Now tracked globally
} client_metrics_t;

/* Global variables */
static apr_hash_t *client_metrics_table;
static apr_thread_mutex_t *table_mutex;
static apr_shm_t *shared_memory;
static int *global_active_connections; // Pointer to shared memory
...
/* Helper function to log client metrics during updates */
static void log_metrics_update(request_rec *r, client_metrics_t *metrics) {
    ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, r, 
        "mod_http_collector [UPDATED]: IP=%s, Method=%s, GlobalActiveConnections=%d, "
        "BytesReceived=%ld, ContentLength=%ld",
        metrics->ip_address, metrics->request_method, *global_active_connections, 
        metrics->bytes_received, metrics->content_length);
}

/* Cleanup function to decrement global active connections */
static apr_status_t decrement_global_connections(void *data) {
    /* Lock the table */
    apr_thread_mutex_lock(table_mutex);

    if (*global_active_connections > 0) {
...
static int http_collector_log_phase(request_rec *r) {
    ...
    if (metrics) {
        ...
        if (test_mode) {
            ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, r, 
                "mod_http_collector [FINAL]: IP=%s, Method=%s, GlobalActiveConnections=%d, "
                "BytesReceived=%ld, ContentLength=%ld, Duration=%ldms, DataRate=%.2fBps",
                metrics->ip_address, metrics->request_method, *global_active_connections,
                metrics->bytes_received, metrics->content_length, metrics->connection_duration, 
...
static int http_collector_handler(request_rec *r) {
    ...
    /* Register cleanup function to decrement global active connections */
    apr_pool_cleanup_register(r->connection->pool, NULL, decrement_global_connections, apr_pool_cleanup_null);

    /* Log metrics update */
    if (test_mode) {
        log_metrics_update(r, metrics);
    }
...
static void http_collector_register_hooks(apr_pool_t *p) {
    ...
    /* Allocate shared memory for global active connections */
    if (apr_shm_create(&shared_memory, sizeof(int), NULL, pool) == APR_SUCCESS) {
        global_active_connections = apr_shm_baseaddr_get(shared_memory);
        *global_active_connections = 0; // Initialize
    }
