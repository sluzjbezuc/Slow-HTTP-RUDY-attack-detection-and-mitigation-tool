...
/* ------------------------------------------------------
 * 4) PER-REQUEST METRICS (stored in request pool)
 * ------------------------------------------------------ */
typedef struct {
...
/* 
 * Input filter: count bytes received; if test_mode, log partial updates ([DYNAMIC]).
 */
                                                ...
                                                apr_off_t readbytes)
{
    ...
    if (rm) {
        ...
        if (test_mode) {
            /* For partial updates, also show concurrency if you want */
...
static int http_collector_log_phase(request_rec *r)
{
    ...
    /* Mark completion, compute duration, data rate */
    ...
    /* Log final metrics if test_mode is on */
    if (test_mode) {
        ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, r,
...
static int http_collector_post_read_request(request_rec *r)
{
    ...
    /* Per-request metrics stored in request pool */
    ...
    /* Attach our input filter to track bytes & partial updates */
    ap_add_input_filter("HTTP_COLLECTOR_FILTER", NULL, r, r->connection);

    /* IMPORTANT: register cleanup on r->pool (request pool),
       so concurrency is decremented once the request ends. */
